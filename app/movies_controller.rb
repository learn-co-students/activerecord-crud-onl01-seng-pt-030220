...
def can_be_created_in_a_block(args = {title: "Home Alone", release_date: 1990})
  #default values = title: "Home Alone", release_date: 1990
  
  movie_object = Movie.create do |m|
    m.title = args[:title]
...
def can_find_using_where_clause_and_be_sorted
  ...
  # release date descendin
  Movie.where("release_date > 2002").order("release_date DESC")
end

def can_be_found_updated_and_saved  
  Movie.create(title: "Awesome Flick") # Updtate the title "Awesome Flick" to "Even Awesomer Flick", save it, then return it
  mo = Movie.find_by(title: 'Awesome Flick')
  mo.title = "Even Awesomer Flick"
  mo.save
end

def can_update_using_update_method  
  Movie.create(title: "Wat?")# Update movie title to "Wat, huh?"
  mo = Movie.find_by(title: 'Wat?')
  mo.update(title: 'Wat, huh?')
end
